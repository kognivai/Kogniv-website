...
router.post("/contact", async (req, res) => {
  ...
  if (message) console.log("[contact] Message:", message);

  try {
    const { client, fromEmail } = await getUncachableResendClient();
    const from = fromEmail ?? "Kogniv <onboarding@resend.dev>";
    console.log("[contact] Sending from:", from);

    const notif = await client.emails.send({
      from,
      to: [ENQUIRY_TO],
      ...
      html: `
        <div style="font-family:sans-serif;max-width:600px;margin:0 auto">
          ...
          <table style="width:100%;border-collapse:collapse">
            ...
            ${message ? `<tr><td style="padding:8px 0;font-weight:bold;vertical-align:top">Message</td><td style="white-space:pre-wrap">${escHtml(message)}</td></tr>` : ""}
          </table>
        </div>
      `,
    });
    ...
    const confirm = await client.emails.send({
      from,
      to: [email],
